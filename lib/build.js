...
const newPluginInstance=(compile, config={})=>{
    ...
    let builder = load();
    if(typeof builder ==='object' && builder.default){
...
async function builder(compile, escOptions, files){
    ...
    const buildAsset = async (asset, options={})=>{
        const input = asset.getAssetFilePath();
        const buildOptions = getEsBuildOptions(input, asset.getAbsoluteResourcePath(), options)
        ...
        Object.keys(outputs).forEach( dist=>{
            ...
            if(input.includes(item.entryPoint)){
                asset.unlink();
                asset.setAttr('dist', compile.normalizePath(path.relative(asset.getBaseDir(), dist)));
            }
        });
    }

    const buildAssets = async(plugin)=>{
        ...
        if(bundle.enable){
            ...
            if(Assets.using){
                ...
                await Promise.allSettled(assets.map(asset=>{
                    const file = compile.normalizePath(asset.getAssetFilePath());
                    assetsRecords.set(file, [asset, plugin.options.bundle]);
