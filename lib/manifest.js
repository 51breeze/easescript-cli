...
module.exports = async(options={})=>{
    ...
    if(fs.existsSync(pkg)){
        ...
        if(esconfig.typings){
            if(Array.isArray(esconfig.typings)){
                additions = esconfig.typings;
            }else if(typeof esconfig.typings==='string'){
                additions = esconfig.typings.split(',').map(file=>file.trim())
            }
        }
    }

    if(Array.isArray(options.additions)){
