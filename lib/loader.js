...
const {isClientSide, isServerSide, getPlugins} = require('./helpers');
...
const loader=(compile, plugins=[], options={})=>{
    ...
    return {
        ...
        async setup(build){
            ...
            const pluginInstances = plugins.map(getPlugins);
            const clients = pluginInstances.filter(item=>isClientSide(item));
            const servers = pluginInstances.filter(item=>isServerSide(item));
            ...
            let isProduction = options.mode === 'production' || process.env.NODE_ENV === 'production';
            let isVueTemplate = false;
            if(builder){
                isVueTemplate = builder.options.format ==='vue-raw' || builder.options.format ==='vue-template' || builder.options.format ==='vue-jsx';
                if(!builder.options.outDir){
                    builder.options.outDir = outdir;
                }
            }
            ...
            build.onLoad({filter, namespace:'file'}, args=>{
                return new Promise( async(resolve,reject)=>{
                    ...
                    if(query.callhook != null && query.action){
                        try{
                            const code = await builder.callHook(compilation, query);
                            resolve({
                                contents:code,
                                loader:'js',
                                errors
                    ...
                    if(!buildGraph){
                        return reject(new Error(`Build error no result. on the "${resource}"`))
                    }
                    ...
                    if(query.type === 'style' || query.type === 'embedAssets'){
                        ...
                        if(!asset){
                            return reject(new Error(`Not found style by "${query.index}". on the "${resource}"`))
                        }
                        content = asset.code;
