...
module.exports = async(options)=>{
    ...
    if(files && !Array.isArray(files)){
        ...
        if(!path.isAbsolute(files)){
            files = path.join(compile.workspace, files)
        }
        if(fs.statSync(files).isDirectory()){
            files = fs.readdirSync(files).filter( file=>{
                if(file==='.' || file==='..')return false;
                if(fs.statSync(file).isFile()){
                    return file.endsWith(compile.options.suffix);
                }
                return false;
            });
        }
    ...
    if(!files){
        ...
        files = files.map(file=>path.isAbsolute(file) ? file : path.join(compile.workspace, file)).filter( file=>{
            if(fs.existsSync(file))return true;
            console.error(`Not found file the '${file}'`)
            return false;
        });
    }
